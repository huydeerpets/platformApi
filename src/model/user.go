...
func UpdateUserPwd(userName, newpwd string) bool {
	o := orm.NewOrm()
	res, err := o.Raw("UPDATE t_platform_user SET password = ? WHERE userName = ?", newpwd, userName).Exec()
	if err == nil {
...
func UpdateLastLoginTime(userName string) bool {
	o := orm.NewOrm()
	res, err := o.Raw("UPDATE t_platform_user SET lastLoginTime = now() WHERE userName = ?", userName).Exec()
	if err == nil {
		num, _ := res.RowsAffected()
		if num > 0 {
			return true
...
func GetUserInfoByAccount(userName string) (*User, error) {
	o := orm.NewOrm()
	qs := o.QueryTable("t_platform_user")
	if userName != "" {
