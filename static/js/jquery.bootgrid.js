...
;(function ($, window, undefined)
{
    ...
    function getRequest()
    {
    		...
    		if(this.sortDictionary) {
    			for(var i in this.sortDictionary) {
    				if(sortStr.length>0)
    					sortStr += ",";
    				sortStr += i + " " + this.sortDictionary[i];
    			}
    		}
        ...
        var searchFields = $(".search").find(".search-field");
		if(searchFields) {
			for(var a =0;a < searchFields.length;a++) {
				var name = $(searchFields[a]).prop("name");
				var v = $(searchFields[a]).val();
				request[""+name] = v;
			}
		}
        //console.log(request);
    ...
    */
    ...
    function renderNoResultsRow()
    {
        ...
        if (this.selection||this.rownumbers)
        {
            count = count + 1;
        }
    ...
    function renderRows(rows)
    {
        if (rows&&rows.length > 0)
        {
            ...
            var startIndex = (that.current-1)*that.rowCount;//0 10
			var endIndex = that.current*that.rowCount;//10 20
			var realTotal = (rows.length<that.rowCount)?rows.length:that.rowCount;
			for(var r=0;r<realTotal;r++) {
				rows[r].rowIndex = (startIndex+1);
				startIndex ++;
			}
            ...
            $.each(rows, function (index, row)
            {
                ...
                if (that.selection)
                {
					...
					if(that.rownumbers) {
						...
						{ type: "checkbox", value: row[that.identifier], checked: selected,text: row.rowIndex}));
					} else {
						selectBox = tpl.select.resolve(getParams.call(that,
						{ type: "checkbox", value: row[that.identifier], checked: selected,text: ""}));
					}
					...
					var selectBox = null;
					if(that.rownumbers) {
                       selectBox = tpl.rownumber.resolve(getParams.call(that,
						{ css: css.rownumberCell,text: row.rowIndex}));
					    cells += tpl.cell.resolve(getParams.call(that, { content: selectBox, css: css.selectCell }));
					}
				}
    ...
    function registerRowEvents(tbody)
    {
        ...
        tbody.off("click" + namespace, "> tr")
            ...
            {
                //e.stopPropagation();//与Clipboard控件冲突

                var $this = $(this),
                    id = (that.identifier == null) ? $this.data("row-id") :that.converter.from($this.data("row-id") + "");
                var row = null;
                if (that.identifier == null) {
                    row = that.currentRows[id];
                } else {
                    if (that.currentRows)
                        row = that.currentRows.first(function (item) { return item[that.identifier] == id; });
                }
                //var row = (that.identifier == null) ? that.currentRows[id] :that.currentRows.first(function (item) { return item[that.identifier] == id; });
			...
			//onmouseover
			tbody.off("mouseover" + namespace, "> tr")
            .on("mouseover" + namespace, "> tr", function(e)
            {
                e.stopPropagation();
                ...
                //var row = (that.identifier == null) ? that.currentRows[id] : that.currentRows.first(function (item) { return item[that.identifier] == id; });
                that.element.trigger("mouseover" + namespace, [$this, row,id]);
            });

			//onmouseover
			tbody.off("mouseout" + namespace, "> tr")
            .on("mouseout" + namespace, "> tr", function(e)
            {
                e.stopPropagation();
                ...
                that.element.trigger("mouseout" + namespace, [$this, row,id]);
            });
    }
    ...
    function renderTableHeader()
    {
        ...
        if (this.selection)
        {
            ...
            var selectBox = (this.options.multiSelect) ?
                tpl.select.resolve(getParams.call(that, { type: "checkbox", value: "all",text:"" })) : "";
    ...
    function showLoading()
    {
        ...
        window.setTimeout(function()
        {
            if (that.element._bgAria("busy") === "true")
            {
                ...
                if (that.selection||that.rownumbers)
                {
                    count = count + 1;
                }
    ...
    var Grid = function(element, options)
    {
		...
		this.rownumbers = this.options.rownumbers?this.options.rownumbers:false;
    ...
    Grid.defaults = {
        ...
        rowCount: [10, 20, 50], // rowCount: [10, 20, 50, -1] rows per page int or array of int (-1 represents "All")
        /**
        ...
        css: {
            ...
            center: "text-center",
            columnHeaderAnchor: "column-header-anchor2", // must be a unique class name or constellation of class names within the column header cell
        ...
        templates: {
            ...
            //footer: "<div id=\"{{ctx.id}}\" class=\"{{css.footer}}\"><div class=\"row\"><div class=\"col-sm-6\"></div><div class=\"col-sm-6\"><p class=\"{{css.actions}}\"></p><p class=\"{{css.pagination}}\"></p></div></div></div>",
			
			footer: "<div id=\"{{ctx.id}}\" class=\"{{css.footer}}\"><div class=\"row\"><div class=\"col-sm-7\"><p class=\"{{css.pagination}}\"></p><p class=\"{{css.actions}}\"></p></div><div class=\"col-sm-5 infoBar\"><p class=\"{{css.infos}}\"></p></div></div></div>",
			
            header: "<div id=\"{{ctx.id}}\" class=\"{{css.header}}\" style=\"display:none;\"></div>",
			...
			select: "<label class=\"{{ctx.type}}-inline\"><input name=\"select\" type=\"{{ctx.type}}\" value=\"{{ctx.value}}\" {{ctx.checked}}>{{ctx.text}}</label>",
			rownumber: "<span class=\"text {{ctx.css}}\">{{ctx.text}}</span>"
        }
    };
     ...
     * @chainable
     **/
    Grid.prototype.setUrl = function(newUrl)
    {
